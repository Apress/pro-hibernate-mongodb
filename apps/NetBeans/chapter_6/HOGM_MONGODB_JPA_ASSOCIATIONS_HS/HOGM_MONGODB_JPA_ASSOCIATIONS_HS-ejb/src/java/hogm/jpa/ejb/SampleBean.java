...
import hogm.jpa.entity.Tournaments;
...
public class SampleBean {
    ...
    @PersistenceContext(unitName = "HOGM_ASSOCIATIONS_HS-ejbPU")
    ...
    public List<Tournaments> selectAllTournamentsAction() {
        ...
        QueryBuilder queryBuilder = fullTextEntityManager.getSearchFactory().buildQueryBuilder().forEntity(Tournaments.class).get();
        org.apache.lucene.search.Sort sort = new Sort(new SortField("tournament", SortField.STRING));
        org.apache.lucene.search.Query query = queryBuilder.all().createQuery();

        // wrap Lucene query in a org.hibernate.FullTextQuery
        FullTextQuery fullTextQuery = fullTextEntityManager.createFullTextQuery(query, Tournaments.class);
        //required by OGM
        fullTextQuery.initializeObjectsWith(ObjectLookupMethod.SKIP, DatabaseRetrievalMethod.FIND_BY_ID);
        // execute search   
        fullTextQuery.setSort(sort);
        List<Tournaments> results = fullTextQuery.getResultList();
